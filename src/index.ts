...
import { TherapyMethodRouter } from './routes/technique.route';

const app = new App({
  port: 8000,
  middlewares: [initCustomLocals(), logger(), cookieParser(), cors()],
  controllers: [new PsychologistRouter(), new AuthRouter(), new SymptomRouter(), new TherapyMethodRouter(), new PatientRouter(), new RecordRouter()],
});
